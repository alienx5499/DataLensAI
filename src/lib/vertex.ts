...
Return EXACT JSON:
{
 ...
 generationConfig: { temperature: 0.2, topP: 0.8, maxOutputTokens: 2048, responseMimeType: 'application/json' },
 });
 ...
 if (errText.includes('BILLING_DISABLED') || errText.includes('PERMISSION_DENIED')) {
 ...
 console.warn('[vertex]', model, 'fetch failed:', e instanceof Error ? e.message : e);
 continue;
 }
 }
...
function generateMock(question: string, profile: unknown, dataSample: unknown[]) {
 ...
 let findings: string = '';
 let limitations = 'Based on sample data. Full dataset may show different patterns.';
 ...
 if (q.includes('top') || q.includes('highest') || q.includes('best') || q.includes('most')) {
 ...
 data = Object.entries(agg).sort((a, b) => b[1] - a[1]).slice(0, 8).map(([name, value]) => ({ name, value }));
 ...
 } else if (q.includes('trend') || q.includes('over time') || q.includes('growth') || q.includes('month')) {
 chartType = 'line';
 const values = sample.slice(0, 10).map((row) => Number(row[valueCol]) || 0);
 ...
 } else if (q.includes('distribut') || q.includes('breakdown') || q.includes('split')) {
 chartType = 'pie';
 const agg: Record<string, number> = {};
 for (const row of sample) {
 ...
 limitations = 'Correlation does not imply causation. External factors may influence results.';
 }

 return {
 chartConfig: { type: chartType, title, xAxis: cols[0]?.name || 'Category', yAxis: cols.find((c) => c.type === 'number')?.name || 'Value', data },
 findings,
 limitations,
 stats: { totalRows, matchingRows: totalRows },
 suggestions: ['Explore another dimension', 'Check trends over time', 'View raw data table'],
 };
}
